...
import static com.amazon.dcv.sm.ui.authserver.providers.PamAuthenticationTest.STATE_KEY;
import static com.amazon.dcv.sm.ui.authserver.providers.PamAuthenticationTest.STATE_VALUE;
...
@TestPropertySource(
        properties = {
            ...
            "authentication.pam.normalize-userid=true"
        })
public class PamAuthenticationWithUserIdNormalizationTest extends BaseTestClass {
    ...
    private static final String TEST_USERNAME_NORMALIZED = "test-user";
    ...
    private Process setupMockProcessBuilderProvider() throws IOException {
        ...
        when(mockProcessBuilder.command(any(), any(), any(), any())).thenReturn(mockProcessBuilder);
        ...
        when(mockProcessBuilder.redirectInput(ProcessBuilder.Redirect.INHERIT)).thenReturn(mockProcessBuilder);
        ...
        when(mockProcessBuilder.redirectInput(ProcessBuilder.Redirect.PIPE)).thenReturn(mockProcessBuilder);
        ...
        try (InputStream mockInputStream = mock(InputStream.class)) {
            when(mockInputStream.readAllBytes()).thenReturn(TEST_USERNAME_NORMALIZED.getBytes(StandardCharsets.UTF_8));
    ...
    public void testValidPamAuthentication() throws Exception {
        ...
        assert spring_security_context.getAuthentication().getPrincipal().equals(TEST_USERNAME_NORMALIZED);
    ...
    @Test
    public void testUserIdNormalizationFailure() throws Exception {
        ...
        when(mockProcess.exitValue()).thenReturn(0).thenReturn(1);
        ...
        assert exception.getMessage().equals("Normalizing userid failed");
    }
}
